...
#include "Input.h"
//the function receives file name and reference to s and t integers (sink and source)
//the function handle creating said graph while calling to all relevant input and validation functions
DirectedGraph *Input::handleInput(int &s, int &t, char *fileName)
{
	...
	int numOfVertex = getValidLine(inFile, isInteger, isPositive);
...
//this function receives file stream and two pointers to boolean functions(arguments for input validation)
...
//this function will skip empty line and returns first line with input
...
//scan string for non interger characters
// function will remove space before checking, therefore the string must contain one word to return true
...
// this function receives input line and parses 3 number from line
// 3 numbers will represent vertex u, vertex v, and edge capacity
// validates inputs will parsing
...
// this function checks graph for self loops
// the check for parallel edges in executes during input parsing.
